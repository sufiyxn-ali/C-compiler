...
static const char* eps_str(const char* s) {
    if(strcmp(s,"\xce\xb5")==0) return "eps";
    return s;
}
...
#define ADD_PROD(N, LHS, ...) do { \
    ...
    for(int _i=0;_i<_len;_i++) strcpy(prods[prod_count].rhs[_i],_rhs[_i]); \
    prod_count++; \
} while(0)
    ...
    ADD_PROD(14, "if_stmt",        "IF", "LPAREN", "bool_expr", "RPAREN", "block", "else_prime");
    ...
    ADD_PROD(37, "bool_term_tail", "LOGICAL_AND", "bool_factor", "bool_term_tail");
...
int is_terminal(const char* s) {
    if(strcmp(s,"\xce\xb5")==0) return 1;
    for(int i=0;TERM_LIST[i];i++) if(strcmp(s,TERM_LIST[i])==0) return 1;
    return 0;
}
int nt_index(const char* s) {
    for(int i=0;i<nt_count;i++) if(strcmp(non_terminals[i],s)==0) return i;
    return -1;
}
int term_index(const char* s) {
    for(int i=0;i<term_count;i++) if(strcmp(terminals[i],s)==0) return i;
    return -1;
}
void build_symbol_lists(void) {
    nt_count=0; term_count=0;
    for(int i=0;i<prod_count;i++)
        if(nt_index(prods[i].lhs)==-1) strcpy(non_terminals[nt_count++],prods[i].lhs);
    for(int i=0;TERM_LIST[i];i++) strcpy(terminals[term_count++],TERM_LIST[i]);
}

/* ========== FIRST SET ========== */
void add_to_first(int ni, const char* s) {
    for(int i=0;i<first_cnt[ni];i++) if(strcmp(first_set[ni][i],s)==0) return;
    strcpy(first_set[ni][first_cnt[ni]++],s);
}
int first_of_sequence(char seq[][MAX_LABEL], int len, char out[][MAX_LABEL], int* out_cnt) {
    *out_cnt=0;
    int all_eps=1;
    for(int i=0;i<len&&all_eps;i++){
        char* sym=seq[i];
        if(strcmp(sym,"\xce\xb5")==0) continue;
        if(is_terminal(sym)){
            int found=0; for(int k=0;k<*out_cnt;k++) if(strcmp(out[k],sym)==0){found=1;break;}
            if(!found) strcpy(out[(*out_cnt)++],sym);
            all_eps=0;
        } else {
            int ni=nt_index(sym); if(ni==-1){all_eps=0;break;}
            int has_eps=0;
            for(int k=0;k<first_cnt[ni];k++){
                if(strcmp(first_set[ni][k],"\xce\xb5")==0){has_eps=1;continue;}
                int found=0; for(int m=0;m<*out_cnt;m++) if(strcmp(out[m],first_set[ni][k])==0){found=1;break;}
                if(!found) strcpy(out[(*out_cnt)++],first_set[ni][k]);
            }
            if(!has_eps) all_eps=0;
        }
    }
    if(all_eps){
        int has_eps=0; for(int k=0;k<*out_cnt;k++) if(strcmp(out[k],"\xce\xb5")==0){has_eps=1;break;}
        if(!has_eps) strcpy(out[(*out_cnt)++],"\xce\xb5");
    }
    return all_eps;
}
void compute_first_sets(void) {
    int changed=1;
    for(int i=0;i<nt_count;i++) first_cnt[i]=0;
    while(changed){
        changed=0;
        for(int p=0;p<prod_count;p++){
            int ni=nt_index(prods[p].lhs); if(ni==-1) continue;
            int before=first_cnt[ni];
            char seq[MAX_RHS][MAX_LABEL]; for(int k=0;k<prods[p].rhs_len;k++) strcpy(seq[k],prods[p].rhs[k]);
            char out[MAX_SYMBOLS][MAX_LABEL]; int out_cnt=0;
            first_of_sequence(seq,prods[p].rhs_len,out,&out_cnt);
            for(int k=0;k<out_cnt;k++) add_to_first(ni,out[k]);
            if(first_cnt[ni]!=before) changed=1;
        }
    }
}
...
void add_to_follow(int ni, const char* s) {
    for(int i=0;i<follow_cnt[ni];i++) if(strcmp(follow_set[ni][i],s)==0) return;
    strcpy(follow_set[ni][follow_cnt[ni]++],s);
}
void compute_follow_sets(void) {
    int changed=1;
    for(int i=0;i<nt_count;i++) follow_cnt[i]=0;
    add_to_follow(nt_index("program"),"$");
    while(changed){
        changed=0;
        for(int p=0;p<prod_count;p++){
            int lhs=nt_index(prods[p].lhs); if(lhs==-1) continue;
            for(int j=0;j<prods[p].rhs_len;j++){
                char* B=prods[p].rhs[j]; if(is_terminal(B)) continue;
                int Bni=nt_index(B); if(Bni==-1) continue;
                int before=follow_cnt[Bni];
                int beta_len=prods[p].rhs_len-j-1;
                if(beta_len>0){
                    char beta[MAX_RHS][MAX_LABEL]; for(int k=0;k<beta_len;k++) strcpy(beta[k],prods[p].rhs[j+1+k]);
                    char out[MAX_SYMBOLS][MAX_LABEL]; int out_cnt=0;
                    int has_eps=first_of_sequence(beta,beta_len,out,&out_cnt);
                    for(int k=0;k<out_cnt;k++) if(strcmp(out[k],"\xce\xb5")!=0) add_to_follow(Bni,out[k]);
                    if(has_eps) for(int k=0;k<follow_cnt[lhs];k++) add_to_follow(Bni,follow_set[lhs][k]);
                } else {
                    for(int k=0;k<follow_cnt[lhs];k++) add_to_follow(Bni,follow_set[lhs][k]);
                }
                if(follow_cnt[Bni]!=before) changed=1;
            }
        }
    }
}
...
/* ========== LL(1) TABLE ========== */
void build_ll1_table(void) {
    for(int i=0;i<nt_count;i++) for(int j=0;j<term_count;j++) ll1_table[i][j]=-1;
    for(int p=0;p<prod_count;p++){
        int ni=nt_index(prods[p].lhs); if(ni==-1) continue;
        char seq[MAX_RHS][MAX_LABEL]; for(int k=0;k<prods[p].rhs_len;k++) strcpy(seq[k],prods[p].rhs[k]);
        char out[MAX_SYMBOLS][MAX_LABEL]; int out_cnt=0;
        ...
        for(int k=0;k<out_cnt;k++){
            if(strcmp(out[k],"\xce\xb5")==0){
                for(int m=0;m<follow_cnt[ni];m++){
                    int ti=term_index(follow_set[ni][m]); if(ti!=-1) ll1_table[ni][ti]=prods[p].num;
                }
            } else {
                int ti=term_index(out[k]); if(ti!=-1) ll1_table[ni][ti]=prods[p].num;
            }
        }
    }
}
...
const char* token_to_terminal(Token* t) {
    ...
    if(strcmp(t->type,"KEYWORD")==0){
        if(strcmp(t->value,"int")==0||strcmp(t->value,"float")==0) return "TYPE";
        if(strcmp(t->value,"if")==0)    return "IF";
        if(strcmp(t->value,"else")==0)  return "ELSE";
        if(strcmp(t->value,"while")==0) return "WHILE";
        if(strcmp(t->value,"print")==0) return "PRINT";
    }
    if(strcmp(t->type,"IDENTIFIER")==0) return "ID";
    if(strcmp(t->type,"INTEGER")==0)    return "INTEGER";
    if(strcmp(t->type,"FLOAT")==0)      return "FLOAT";
    if(strcmp(t->type,"ASSIGNMENT")==0) return "ASSIGNMENT";
    if(strcmp(t->type,"RELATIONAL")==0) return "REL_OP";
    if(strcmp(t->type,"ARITHMETIC")==0){
        if(strcmp(t->value,"+")==0) return "PLUS";
        if(strcmp(t->value,"-")==0) return "MINUS";
        if(strcmp(t->value,"*")==0) return "MULT";
        if(strcmp(t->value,"/")==0) return "DIV";
        if(strcmp(t->value,"%")==0) return "MOD";
    }
    if(strcmp(t->type,"LOGICAL")==0){
        if(strcmp(t->value,"!")==0)  return "NOT";
        if(strcmp(t->value,"&&")==0) return "LOGICAL_AND";
        if(strcmp(t->value,"||")==0) return "LOGICAL_OR";
    }
    if(strcmp(t->type,"DELIMITER")==0){
        if(strcmp(t->value,"(")==0) return "LPAREN";
        if(strcmp(t->value,")")==0) return "RPAREN";
        if(strcmp(t->value,";")==0) return "SEMI";
        if(strcmp(t->value,"{")==0) return "LBRACE";
        if(strcmp(t->value,"}")==0) return "RBRACE";
    }
    return t->type;
}
...
void print_productions(void) {
    ...
    for(int i=0;i<prod_count;i++){
        ...
        if(strcmp(prods[i].rhs[0],"\xce\xb5")==0) printf("eps");
        else for(int j=0;j<prods[i].rhs_len;j++) printf("%s ",eps_str(prods[i].rhs[j]));
        printf("\n");
    }
    printf("\n");
}
void print_first_sets(void) {
    ...
    for(int i=0;i<nt_count;i++){
        ...
        for(int j=0;j<first_cnt[i];j++) printf("%s ",eps_str(first_set[i][j]));
        printf("}\n");
    }
    printf("\n");
}
void print_follow_sets(void) {
    ...
    for(int i=0;i<nt_count;i++){
        ...
        for(int j=0;j<follow_cnt[i];j++) printf("%s ",eps_str(follow_set[i][j]));
        printf("}\n");
    }
    printf("\n");
}
void print_ll1_table(void) {
    ...
    for(int pass=0;pass<passes;pass++){
        ...
        if(col_end>term_count) col_end=term_count;
        printf("+-"); for(int k=0;k<NT_W;k++) printf("-"); printf("-+");
        for(int j=col_start;j<col_end;j++){for(int k=0;k<COL_W;k++) printf("-"); printf("+");}
        printf("\n| %-*s |",NT_W,"Non-Terminal");
        for(int j=col_start;j<col_end;j++) printf(" %-*s|",COL_W-1,terminals[j]);
        printf("\n+-"); for(int k=0;k<NT_W;k++) printf("-"); printf("-+");
        for(int j=col_start;j<col_end;j++){for(int k=0;k<COL_W;k++) printf("-"); printf("+");}
        printf("\n");
        for(int i=0;i<nt_count;i++){
            printf("| %-*s |",NT_W,non_terminals[i]);
            for(int j=col_start;j<col_end;j++){
                if(ll1_table[i][j]!=-1){char cell[16]; snprintf(cell,sizeof(cell),"P%d",ll1_table[i][j]); printf(" %-*s|",COL_W-1,cell);}
                else printf(" %-*s|",COL_W-1,"");
            }
            printf("\n");
        }
        printf("+-"); for(int k=0;k<NT_W;k++) printf("-"); printf("-+");
        for(int j=col_start;j<col_end;j++){for(int k=0;k<COL_W;k++) printf("-"); printf("+");}
        printf("\n\n");
        if(pass<passes-1) printf("  (continued)\n\n");
    }
...
/* ========== LL(1) PARSER ========== */
int ll1_parse_statement(Node* stmt) {
    Token stoks[MAX_TOKENS]; int slen=0;
    node_to_tokens(stmt,stoks,&slen);
    if(!quiet_mode){
        printf("\n================================================================================\n");
        printf("LL(1) PARSER - STATEMENT TRACE\n");
        printf("================================================================================\n\n");
        printf("Input: "); for(int i=0;i<slen;i++) printf("%s ",stoks[i].value); printf("\n\n");
    }
    char input[MAX_TOKENS+2][MAX_LABEL]; int input_lines[MAX_TOKENS+2]; int input_len=0;
    for(int i=0;i<slen;i++){
        strcpy(input[input_len],token_to_terminal(&stoks[i]));
        input_lines[input_len]=stoks[i].line; input_len++;
    }
    strcpy(input[input_len],"$"); input_lines[input_len]=(slen>0?stoks[slen-1].line:0); input_len++;
    char stack[MAX_STACK][MAX_LABEL]; int top=-1;
    strcpy(stack[++top],"$"); strcpy(stack[++top],"stmt");
    int ip=0,step=1,success=1,ll1_errors=0;
    if(!quiet_mode){
        printf("%-5s  %-42s  %-30s  %s\n","Step","Stack (top-bottom)","Remaining Input","Action");
        printf("-----  ------------------------------------------  ------------------------------  ---------------------------------\n");
    }
    while(top>=0&&step<300){
        char* X=stack[top]; char* a=(ip<input_len)?input[ip]:"$";
        char ss[256]=""; int spos=0;
        for(int i=top;i>=0&&spos<250;i--){int l=strlen(stack[i]); memcpy(ss+spos,stack[i],l); spos+=l; if(i>0&&spos<249){ss[spos++]=' ';}} ss[spos]='\0';
        char is[128]=""; int ipos=0;
        for(int i=ip;i<input_len&&i<ip+5&&ipos<120;i++){int l=strlen(input[i]); memcpy(is+ipos,input[i],l); ipos+=l; if(i<input_len-1&&ipos<119){is[ipos++]=' ';}}
        if(ip+5<input_len){strcpy(is+ipos,"..."); ipos+=3;} is[ipos]='\0';
        if(!quiet_mode) printf("%-5d  %-42s  %-30s  ",step,ss,is);
        step++;
        if(strcmp(X,"$")==0&&strcmp(a,"$")==0){if(!quiet_mode) printf("ACCEPT\n"); break;}
        if(is_terminal(X)){
            if(strcmp(X,a)==0){if(!quiet_mode) printf("Match '%s'\n",X); top--; ip++;}
            else if(strcmp(X,"\xce\xb5")==0){if(!quiet_mode) printf("Pop eps\n"); top--;}
            else{int ln=(ip<input_len)?input_lines[ip]:(slen>0?stoks[slen-1].line:0); if(!quiet_mode) printf("*** ERROR: expected '%s', found '%s' (line %d)\n",X,a,ln); ll1_errors++; success=0; break;}
        } else {
            int ni=nt_index(X),ti=term_index(a);
            if(ni<0||ti<0||ll1_table[ni][ti]==-1){int ln=(ip<input_len)?input_lines[ip]:(slen>0?stoks[slen-1].line:0); if(!quiet_mode) printf("*** ERROR: no production for [%s,%s] (line %d)\n",X,a,ln); ll1_errors++; success=0; break;}
            int pnum=ll1_table[ni][ti];
            for(int p=0;p<prod_count;p++){
                if(prods[p].num==pnum){
                    if(!quiet_mode){
                        printf("Apply P%d: %s -> ",pnum,prods[p].lhs);
                        if(strcmp(prods[p].rhs[0],"\xce\xb5")==0) printf("eps");
                        else for(int j=0;j<prods[p].rhs_len;j++) printf("%s ",eps_str(prods[p].rhs[j]));
                        printf("\n");
                    }
                    top--;
                    if(strcmp(prods[p].rhs[0],"\xce\xb5")!=0) for(int j=prods[p].rhs_len-1;j>=0;j--) strcpy(stack[++top],prods[p].rhs[j]);
                    break;
                }
            }
        }
    ...
    if(!quiet_mode){
        printf("\n");
        if(ll1_errors>0) printf("[FAILED] LL(1) Parser: %d error(s) detected.\n\n",ll1_errors);
        else if(success) printf("[SUCCESS] LL(1) Parser accepted the statement.\n\n");
        else printf("[FAILED] LL(1) Parser: step limit reached.\n\n");
    }
    return success&&ll1_errors==0;
}

/* ========== SHIFT-REDUCE PARSER ========== */
typedef struct { char sym[MAX_LABEL]; int line; } SRItem;
SRItem sr_stack[MAX_STACK];
int    sr_top = -1;

static void sr_push(const char* s, int ln){
    if(sr_top<MAX_STACK-1){ sr_top++; strcpy(sr_stack[sr_top].sym,s); sr_stack[sr_top].line=ln; }
}
static void sr_pop(void){ if(sr_top>=0) sr_top--; }
...
static int sr_try_reduce(const char* la, char* desc) {
    ...
    if(sr_top >= 4 && sr_check(4,"TYPE") && sr_check(3,"ID") &&
       sr_check(2,"ASSIGNMENT") && sr_check(1,"arith_expr") && sr_check(0,"SEMI")) {
        int ln = sr_stack[sr_top-2].line;
        sr_pop(); sr_pop(); sr_pop();
        sr_push("decl_prime", ln);
        sprintf(desc, "[11] decl_prime -> = arith_expr ;"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"TYPE") && sr_check(1,"ID") && sr_check(0,"SEMI")) {
        int ln = sr_stack[sr_top].line;
        sr_pop();
        sr_push("decl_prime", ln);
        sprintf(desc, "[12] decl_prime -> ;"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"TYPE") && sr_check(1,"ID") && sr_check(0,"decl_prime")) {
        int ln = sr_stack[sr_top-2].line;
        sr_pop(); sr_pop(); sr_pop();
        sr_push("decl_stmt", ln);
        sprintf(desc, "[10] decl_stmt -> TYPE ID decl_prime"); return 1;
    }
    if(sr_check(0,"decl_stmt")) {
        int ln = sr_stack[sr_top].line;
        sr_pop(); sr_push("stmt", ln);
        sprintf(desc, "[4] stmt -> decl_stmt"); return 1;
    }
    ...
    if(sr_top >= 3 && sr_check(3,"ID") && sr_check(2,"ASSIGNMENT") &&
       sr_check(1,"arith_expr") && sr_check(0,"SEMI")) {
        if(!(sr_top >= 4 && sr_check(4,"TYPE"))) {
            ...
            sr_pop(); sr_pop(); sr_pop(); sr_pop();
            sr_push("assign_stmt", ln);
            sprintf(desc, "[13] assign_stmt -> ID = arith_expr ;"); return 1;
        }
    }
    if(sr_check(0,"assign_stmt")) {
        int ln = sr_stack[sr_top].line;
        sr_pop(); sr_push("stmt", ln);
        sprintf(desc, "[5] stmt -> assign_stmt"); return 1;
    }

    /* ===== C. FACTOR from terminal atoms ===== */
    if(sr_check(0,"ID")) {
        if(sr_top >= 1 && sr_check(1,"TYPE")) return 0;
        if(strcmp(la,"ASSIGNMENT")==0) return 0;
        int ln = sr_stack[sr_top].line;
        sr_pop(); sr_push("factor", ln);
        sprintf(desc, "[30] factor -> ID"); return 1;
    }
    if(sr_check(0,"INTEGER")) {
        int ln = sr_stack[sr_top].line;
        sr_pop(); sr_push("factor", ln);
        sprintf(desc, "[31] factor -> INTEGER"); return 1;
    }
    if(sr_check(0,"FLOAT")) {
        int ln = sr_stack[sr_top].line;
        sr_pop(); sr_push("factor", ln);
        sprintf(desc, "[32] factor -> FLOAT"); return 1;
    }

    /* ===== D. factor -> ( arith_expr ) ===== */
    if(sr_top >= 2 && sr_check(2,"LPAREN") && sr_check(1,"arith_expr") && sr_check(0,"RPAREN")) {
        int is_ctrl = 0;
        if(sr_top >= 3) {
        ...
        if(!is_ctrl) {
            ...
            sr_pop(); sr_pop(); sr_pop();
            sr_push("factor", ln);
            sprintf(desc, "[29] factor -> ( arith_expr )"); return 1;
        }
    }

    /* ===== E. TERM_TAIL ===== */
    if(sr_top >= 2 && sr_check(2,"MULT") && sr_check(1,"factor") && sr_check(0,"term_tail")) {
        int ln = sr_stack[sr_top-2].line; sr_pop(); sr_pop(); sr_pop();
        sr_push("term_tail", ln); sprintf(desc, "[25] term_tail -> * factor term_tail"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"DIV") && sr_check(1,"factor") && sr_check(0,"term_tail")) {
        int ln = sr_stack[sr_top-2].line; sr_pop(); sr_pop(); sr_pop();
        sr_push("term_tail", ln); sprintf(desc, "[26] term_tail -> / factor term_tail"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"MOD") && sr_check(1,"factor") && sr_check(0,"term_tail")) {
        int ln = sr_stack[sr_top-2].line; sr_pop(); sr_pop(); sr_pop();
        sr_push("term_tail", ln); sprintf(desc, "[27] term_tail -> %% factor term_tail"); return 1;
    }
    if(sr_check(0,"factor")) {
        if(strcmp(la,"MULT")!=0 && strcmp(la,"DIV")!=0 && strcmp(la,"MOD")!=0) {
            int ln = sr_stack[sr_top].line;
            sr_push("term_tail", ln);
            sprintf(desc, "[28] term_tail -> eps"); return 1;
        }
    }

    /* ===== F. TERM ===== */
    if(sr_top >= 1 && sr_check(1,"factor") && sr_check(0,"term_tail")) {
        int ln = sr_stack[sr_top-1].line; sr_pop(); sr_pop();
        sr_push("term", ln); sprintf(desc, "[24] term -> factor term_tail"); return 1;
    }

    /* ===== G. ARITH_EXPR_TAIL ===== */
    if(sr_top >= 2 && sr_check(2,"PLUS") && sr_check(1,"term") && sr_check(0,"arith_expr_tail")) {
        int ln = sr_stack[sr_top-2].line; sr_pop(); sr_pop(); sr_pop();
        sr_push("arith_expr_tail", ln); sprintf(desc, "[21] arith_expr_tail -> + term arith_expr_tail"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"MINUS") && sr_check(1,"term") && sr_check(0,"arith_expr_tail")) {
        int ln = sr_stack[sr_top-2].line; sr_pop(); sr_pop(); sr_pop();
        sr_push("arith_expr_tail", ln); sprintf(desc, "[22] arith_expr_tail -> - term arith_expr_tail"); return 1;
    }
    if(sr_check(0,"term")) {
        if(strcmp(la,"PLUS")!=0 && strcmp(la,"MINUS")!=0) {
            int ln = sr_stack[sr_top].line;
            sr_push("arith_expr_tail", ln);
            sprintf(desc, "[23] arith_expr_tail -> eps"); return 1;
        }
    }
    ...
    if(sr_top >= 1 && sr_check(1,"term") && sr_check(0,"arith_expr_tail")) {
        int ln = sr_stack[sr_top-1].line; sr_pop(); sr_pop();
        sr_push("arith_expr", ln); sprintf(desc, "[20] arith_expr -> term arith_expr_tail"); return 1;
    }

    /* ===== I. BOOLEAN EXPRESSIONS ===== */
    if(sr_check(0,"REL_OP")) {
        int ln = sr_stack[sr_top].line; sr_pop(); sr_push("rel_op", ln);
        sprintf(desc, "[42] rel_op -> REL_OP"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"arith_expr") && sr_check(1,"rel_op") && sr_check(0,"arith_expr")) {
        int ln = sr_stack[sr_top-2].line;
        sr_pop(); sr_pop(); sr_pop();
        sr_push("bool_factor", ln);
        sprintf(desc, "[39] bool_factor -> arith_expr rel_op arith_expr"); return 1;
    }
    if(sr_top >= 1 && sr_check(1,"NOT") && sr_check(0,"bool_factor")) {
        int ln = sr_stack[sr_top-1].line;
        sr_pop(); sr_pop();
        sr_push("bool_factor", ln);
        sprintf(desc, "[41] bool_factor -> NOT bool_factor"); return 1;
    }
    if(sr_top >= 2 && sr_check(2,"LPAREN") && sr_check(1,"bool_expr") && sr_check(0,"RPAREN")) {
        int is_control = 0;
        ...
        if(!is_control) {
            ...
            sr_pop(); sr_pop(); sr_pop();
            sr_push("bool_factor", ln);
            sprintf(desc, "[40] bool_factor -> ( bool_expr )"); return 1;
        }
    }
    if(sr_check(0,"bool_factor")) {
        if(strcmp(la,"LOGICAL_AND")!=0) {
            ...
            sprintf(desc, "[38] bool_term_tail -> eps"); return 1;
        }
    }
    if(sr_top >= 2 && sr_check(2,"LOGICAL_AND") && sr_check(1,"bool_factor") && sr_check(0,"bool_term_tail")) {
        int ln = sr_stack[sr_top-2].line;
        sr_pop(); sr_pop(); sr_pop();
        sr_push("bool_term_tail", ln);
        sprintf(desc, "[37] bool_term_tail -> && bool_factor bool_term_tail"); return 1;
    }
    if(sr_top >= 1 && sr_check(1,"bool_factor") && sr_check(0,"bool_term_tail")) {
        int ln = sr_stack[sr_top-1].line;
        sr_pop(); sr_pop();
        sr_push("bool_term", ln);
        sprintf(desc, "[36] bool_term -> bool_factor bool_term_tail"); return 1;
    }
    if(sr_check(0,"bool_term")) {
        if(strcmp(la,"LOGICAL_OR")!=0) {
            int ln = sr_stack[sr_top].line;
            sr_push("bool_expr_tail", ln);
            sprintf(desc, "[35] bool_expr_tail -> eps"); return 1;
        }
    }
    if(sr_top >= 2 && sr_check(2,"LOGICAL_OR") && sr_check(1,"bool_term") && sr_check(0,"bool_expr_tail")) {
        int ln = sr_stack[sr_top-2].line;
        sr_pop(); sr_pop(); sr_pop();
        sr_push("bool_expr_tail", ln);
        sprintf(desc, "[34] bool_expr_tail -> || bool_term bool_expr_tail"); return 1;
    }
    if(sr_top >= 1 && sr_check(1,"bool_term") && sr_check(0,"bool_expr_tail")) {
        int ln = sr_stack[sr_top-1].line;
        sr_pop(); sr_pop();
        sr_push("bool_expr", ln);
        sprintf(desc, "[33] bool_expr -> bool_term bool_expr_tail"); return 1;
    }

    /* ===== J. PRINT STATEMENT ===== */
    if(sr_top >= 4 && sr_check(4,"PRINT") && sr_check(3,"LPAREN") && sr_check(2,"arith_expr") &&
       sr_check(1,"RPAREN") && sr_check(0,"SEMI")) {
        int ln = sr_stack[sr_top-4].line;
        sr_pop(); sr_pop(); sr_pop(); sr_pop(); sr_pop();
        sr_push("print_stmt", ln);
        sprintf(desc, "[18] print_stmt -> print ( arith_expr ) ;"); return 1;
    }
    if(sr_check(0,"print_stmt")) {
        int ln = sr_stack[sr_top].line; sr_pop(); sr_push("stmt", ln);
        sprintf(desc, "[8] stmt -> print_stmt"); return 1;
    }

    /* ===== K. BLOCK ===== */
    if(sr_top >= 2 && sr_check(2,"LBRACE") && sr_check(1,"stmt_list") && sr_check(0,"RBRACE")) {
        int ln = sr_stack[sr_top-2].line;
        sr_pop(); sr_pop(); sr_pop();
        sr_push("block", ln);
        sprintf(desc, "[19] block -> { stmt_list }"); return 1;
    }
    ...
    if(sr_top >= 1 && sr_check(1,"stmt") && sr_check(0,"stmt_list")) {
        ...
        sr_pop(); sr_pop();
        sr_push("stmt_list", ln);
        sprintf(desc, "[2] stmt_list -> stmt stmt_list"); return 1;
    }
    if(sr_check(0,"stmt") && (strcmp(la,"RBRACE")==0 || strcmp(la,"$")==0)) {
        int ln = sr_stack[sr_top].line;
        sr_push("stmt_list", ln);
        sr_pop(); sr_pop();
        sr_push("stmt_list", ln);
        sprintf(desc, "[2+3] stmt_list -> stmt eps"); return 1;
    }

    /* ===== M. WHILE STATEMENT ===== */
    if(sr_top >= 4 && sr_check(4,"WHILE") && sr_check(3,"LPAREN") && sr_check(2,"bool_expr") &&
       sr_check(1,"RPAREN") && sr_check(0,"block")) {
        int ln = sr_stack[sr_top-4].line;
        sr_pop(); sr_pop(); sr_pop(); sr_pop(); sr_pop();
        sr_push("while_stmt", ln);
        sprintf(desc, "[17] while_stmt -> while ( bool_expr ) block"); return 1;
    }
    if(sr_check(0,"while_stmt")) {
        int ln = sr_stack[sr_top].line; sr_pop(); sr_push("stmt", ln);
        sprintf(desc, "[7] stmt -> while_stmt"); return 1;
    }
    ...
    if(sr_top >= 1 && sr_check(1,"ELSE") && sr_check(0,"block")) {
        ...
        sprintf(desc, "[15] else_prime -> else block"); return 1;
    }
    if(sr_top >= 5 && sr_check(5,"IF") && sr_check(4,"LPAREN") && sr_check(3,"bool_expr") &&
       sr_check(2,"RPAREN") && sr_check(1,"block") && sr_check(0,"else_prime")) {
        int ln = sr_stack[sr_top-5].line;
        sr_pop(); sr_pop(); sr_pop(); sr_pop(); sr_pop(); sr_pop();
        sr_push("if_stmt", ln);
        sprintf(desc, "[14] if_stmt -> if ( bool_expr ) block else_prime"); return 1;
    }
    if(sr_top >= 4 && sr_check(4,"IF") && sr_check(3,"LPAREN") && sr_check(2,"bool_expr") &&
       sr_check(1,"RPAREN") && sr_check(0,"block")) {
        if(strcmp(la,"ELSE")!=0) {
            int ln = sr_stack[sr_top-4].line;
            sr_pop(); sr_pop(); sr_pop(); sr_pop(); sr_pop();
            sr_push("if_stmt", ln);
            sprintf(desc, "[14+16] if_stmt -> if ( bool_expr ) block (no else)"); return 1;
        }
    }
    if(sr_check(0,"if_stmt")) {
        int ln = sr_stack[sr_top].line;
        sr_pop(); sr_push("stmt", ln);
        sprintf(desc, "[6] stmt -> if_stmt"); return 1;
    }
    ...
    if(sr_check(0,"block")) {
        ...
        for(int i=sr_top-1;i>=0;i--){
            if(strcmp(sr_stack[i].sym,"IF")==0 || strcmp(sr_stack[i].sym,"WHILE")==0 ||
               strcmp(sr_stack[i].sym,"ELSE")==0) {
                dominated = 1; break;
            }
            if(strcmp(sr_stack[i].sym,"LBRACE")==0) break;
        }
        if(!dominated && (strcmp(la,"RBRACE")==0 || strcmp(la,"$")==0 ||
           strcmp(la,"TYPE")==0 || strcmp(la,"ID")==0 || strcmp(la,"IF")==0 ||
           strcmp(la,"WHILE")==0 || strcmp(la,"PRINT")==0 || strcmp(la,"LBRACE")==0)) {
            int ln = sr_stack[sr_top].line;
            sr_pop(); sr_push("stmt", ln);
            sprintf(desc, "[9] stmt -> block"); return 1;
        }
    }
    return 0;
}
...
int sr_parse_statement(Node* stmt) {
    ...
    while(step<500 && ip<=input_len){
        ...
        char ss[512]=""; int spos=0;
        for(int i=0;i<=sr_top&&spos<450;i++){
            int l=strlen(sr_stack[i].sym);
            if(spos+l<450){memcpy(ss+spos,sr_stack[i].sym,l); spos+=l; if(i<sr_top&&spos<449){ss[spos++]=' ';}}
        }
        if(sr_top<0){strcpy(ss,"(empty)"); spos=7;} ss[spos]='\0';
        char is[256]=""; int ipos=0;
        for(int i=ip;i<input_len&&i<ip+6&&ipos<200;i++){
            int l=strlen(input[i]);
            if(ipos+l<200){memcpy(is+ipos,input[i],l); ipos+=l; if(i<input_len-1&&ipos<199){is[ipos++]=' ';}}
        }
        if(ip+6<input_len&&ipos<196){strcpy(is+ipos,"..."); ipos+=3;} is[ipos]='\0';
        if(!quiet_mode) printf("%-5d  %-45s  %-30s  ",step,ss,is);
        step++;
        if(sr_top==0 && strcmp(sr_stack[0].sym,"stmt")==0 && strcmp(la,"$")==0){
            if(!quiet_mode) printf("ACCEPT\n"); break;
        }
        char desc[256]="";
        int reduced = 0;
        while(sr_try_reduce(la,desc)){
            if(!quiet_mode) printf("Reduce: %s\n",desc);
            reduced = 1;
        ...
        if(reduced) continue;
        if(strcmp(la,"$")!=0){
            if(!quiet_mode) printf("Shift: %s\n",la);
            sr_push(la,(ip<input_len)?input_lines[ip]:0); ip++;
        } else {
            if(!quiet_mode) printf("*** ERROR: cannot reduce or shift at '$'\n");
            sr_errors++; success=0; break;
        }
    }
    accept:
    if(step>=500){ if(!quiet_mode) printf("*** ERROR: step limit reached\n"); success=0; sr_errors++; }
    if(!quiet_mode){
        printf("\n");
        if(sr_errors>0)      printf("[FAILED] Shift-Reduce Parser: %d error(s) detected.\n\n",sr_errors);
        else if(success)     printf("[SUCCESS] Shift-Reduce Parser accepted the statement.\n\n");
        else                 printf("[FAILED] Shift-Reduce Parser did not accept.\n\n");
    }
...
void node_to_tokens(Node* node, Token* out, int* out_len) {
    if(!node) return;
    if(node->value[0]!='\0'){
        Token t; t.line=node->line;
        if(strcmp(node->label,"ID")==0) strcpy(t.type,"IDENTIFIER");
        else if(strcmp(node->label,"INTEGER")==0) strcpy(t.type,"INTEGER");
        else if(strcmp(node->label,"FLOAT")==0) strcpy(t.type,"FLOAT");
        else if(strcmp(node->label,"TYPE")==0) strcpy(t.type,"KEYWORD");
        else if(strcmp(node->label,"IF")==0||strcmp(node->label,"ELSE")==0||
                strcmp(node->label,"WHILE")==0||strcmp(node->label,"PRINT")==0) strcpy(t.type,"KEYWORD");
        else if(strcmp(node->label,"ASSIGNMENT")==0) strcpy(t.type,"ASSIGNMENT");
        else if(strcmp(node->label,"ARITHMETIC")==0||strcmp(node->label,"PLUS")==0||
                strcmp(node->label,"MINUS")==0||strcmp(node->label,"MULT")==0||
                strcmp(node->label,"DIV")==0||strcmp(node->label,"MOD")==0) strcpy(t.type,"ARITHMETIC");
        else if(strcmp(node->label,"LOGICAL")==0||strcmp(node->label,"NOT")==0||
                strcmp(node->label,"LOGICAL_AND")==0||strcmp(node->label,"LOGICAL_OR")==0) strcpy(t.type,"LOGICAL");
        else if(strcmp(node->label,"REL_OP")==0) strcpy(t.type,"RELATIONAL");
        else if(strcmp(node->label,"LPAREN")==0||strcmp(node->label,"RPAREN")==0||
                strcmp(node->label,"LBRACE")==0||strcmp(node->label,"RBRACE")==0||
                strcmp(node->label,"SEMI")==0) strcpy(t.type,"DELIMITER");
        else strcpy(t.type,node->label);
        strcpy(t.value,node->value); out[(*out_len)++]=t;
    }
    for(int i=0;i<node->child_count;i++) node_to_tokens(node->children[i],out,out_len);
}
...
void print_error(const char* msg, int line){ 
    ...
    if(error_msg_total < MAX_STMTS) {
        sprintf(error_messages[error_msg_total], ">> Syntax Error at line %d: %s", line, msg);
        error_msg_total++;
    }
    error_count++; 
}
...
Token* expect_type(const char* expected){
    ...
    if(!t||strcmp(t->type,expected)!=0){
        char err[128]; sprintf(err,"Expected type %s, got %s",expected,t?t->type:"EOF");
...
Token* expect_delim(const char* expected){
    Token* t=current_tok();
    if(!t){char err[64]; sprintf(err,"Expected '%s', got EOF",expected); print_error(err,last_line); return NULL;}
    if(strcmp(t->type,"DELIMITER")!=0||strcmp(t->value,expected)!=0){
        char err[128]; sprintf(err,"Expected '%s', got '%s'",expected,t->value); print_error(err,last_line); return NULL;
    }
    last_line=t->line; pos++; return t;
}
void skip_to_next_statement(void){
    while(pos<token_count){
        ...
        if(strcmp(t->type,"KEYWORD")==0){
            ...
            pos++; continue;
        } else if(strcmp(t->type,"DELIMITER")==0&&strcmp(t->value,"}")==0){ pos++; return; }
        pos++;
    }
}
void synchronize(void){
...
void get_statement_source(int start_pos, int end_pos, char* buffer){
    ...
    for(int i=start_pos;i<=end_pos&&i<token_count;i++){
        strcat(buffer,tokens[i].value); if(i<end_pos) strcat(buffer," ");
    }
}
...
Node* parse_factor(void){
    ...
    if(strcmp(t->value,"(")==0){
        expect_delim("("); Node* expr=parse_expr(); if(!expr) return NULL;
        add_child(node,expr); if(!expect_delim(")")) return NULL;
...
Node* parse_term(void){
    ...
    while(current_tok()){
        ...
        if(strcmp(op->type,"ARITHMETIC")==0&&(strcmp(op->value,"*")==0||strcmp(op->value,"/")==0||strcmp(op->value,"%")==0)){
            consume_tok(); Node* right=parse_factor(); if(!right) return NULL;
            Node* parent=create_node("term",NULL,op->line);
            add_child(parent,node); add_child(parent,create_node("ARITHMETIC",op->value,op->line)); add_child(parent,right); node=parent;
        } else break;
    }
    return node;
}
Node* parse_expr(void){
    ...
    while(current_tok()){
        Token* op=current_tok();
        if(strcmp(op->type,"ARITHMETIC")==0&&(strcmp(op->value,"+")==0||strcmp(op->value,"-")==0)){
            consume_tok(); Node* right=parse_term(); if(!right) return NULL;
...
Node* parse_bool_atom(void){
    ...
    if(strcmp(cur->value,"!")==0){
        Node* node=create_node("bool_expr",NULL,cur->line); consume_tok();
        add_child(node,create_node("NOT","!",last_line)); Node* next=parse_bool_atom(); if(next) add_child(node,next); return node;
    } else if(strcmp(cur->value,"(")==0){
        expect_delim("("); Node* node=parse_bool_expr(); expect_delim(")"); return node;
    } else {
        Node* n=create_node("bool_expr",NULL,cur->line); Node* expr=parse_expr(); if(expr) add_child(n,expr);
        Token* t=current_tok();
        if(t&&strcmp(t->type,"RELATIONAL")==0){ consume_tok(); add_child(n,create_node("REL_OP",t->value,t->line)); Node* right=parse_expr(); if(right) add_child(n,right); }
        return n;
    }
}
Node* parse_bool_expr(void){
    ...
    while(current_tok()){
        ...
        if(t&&strcmp(t->type,"LOGICAL")==0){
            ...
            add_child(parent,node); add_child(parent,create_node("LOGICAL",t->value,t->line)); consume_tok();
            Node* right=parse_bool_atom(); if(right) add_child(parent,right); node=parent;
        } else break;
    }
    return node;
}
Node* parse_decl_statement(void){
    ...
    if(current_tok()&&strcmp(current_tok()->type,"ASSIGNMENT")==0){
        t=consume_tok(); add_child(node,create_node("ASSIGNMENT",t->value,t->line));
...
Node* parse_assign_statement(void){
    ...
    t=expect_type("IDENTIFIER"); if(!t) return NULL;
    add_child(node,create_node("ID",t->value,t->line));
    Token* at=expect_type("ASSIGNMENT"); if(!at){synchronize(); strcpy(node->rd_error, last_rd_error); return node;}
...
Node* parse_if_statement(void){
    ...
    if(strcmp(t->value,"if")!=0){print_error("expected 'if'",last_line); return NULL;} consume_tok();
...
Node* parse_while_statement(void){
    ...
    if(strcmp(t->value,"while")!=0){print_error("expected 'while'",last_line); return NULL;} consume_tok();
...
Node* parse_print_statement(void){
    ...
    if(strcmp(t->value,"print")!=0){print_error("expected 'print'",last_line); return NULL;} consume_tok();
...
Node* parse_statement(void){
    ...
    if(strcmp(t->type,"KEYWORD")==0){
        ...
        char err[64]; sprintf(err,"Unknown keyword '%s'",t->value); print_error(err,t->line);
    ...
    char err[64]; sprintf(err,"Unexpected token '%s'",t->value); print_error(err,t->line);
    return parse_error_statement();
}
Node* parse_statement_list(void){
...
void add_child(Node* p, Node* c){ if(p&&c&&p->child_count<MAX_CHILDREN) p->children[p->child_count++]=c; }
void print_tree(Node* node, int indent){
    if(!node) return;
...
void collect_statements(Node* node){
    if(!node) return;
    if(strcmp(node->label,"decl_stmt")==0||strcmp(node->label,"assign_stmt")==0||
       strcmp(node->label,"if_stmt")==0  ||strcmp(node->label,"while_stmt")==0 ||
       strcmp(node->label,"print_stmt")==0||strcmp(node->label,"error_stmt")==0){
        all_stmts[stmt_count++]=node;
    }
    for(int i=0;i<node->child_count;i++) collect_statements(node->children[i]);
}
void node_to_string(Node* node, char* buf, int* len){
    if(!node) return;
    if(node->value[0]!='\0'&&strcmp(node->label,"ERROR")!=0){
        int vl=strlen(node->value); memcpy(buf+*len,node->value,vl); *len+=vl; buf[(*len)++]=' ';
    }
    for(int i=0;i<node->child_count;i++) node_to_string(node->children[i],buf,len);
}
void print_statement_source(Node* node){
    if(node->source[0] != '\0') {
...
void leftmost_derivation(Node* root){
    ...
    Node* sent[MAX_TOKENS]; int count=1; sent[0]=root; int step=1;
    printf("  Leftmost derivation:\n\n");
    while(1){
        printf("Step %d => ",step++);
        for(int i=0;i<count;i++){
            if(sent[i]->value[0]!='\0') printf("%s ",sent[i]->value); 
            else printf("%s ",sent[i]->label);
        }
        printf("\n");
        int lpos=-1; 
        for(int i=0;i<count;i++) if(sent[i]->child_count>0){lpos=i; break;}
        if(lpos==-1) break;
        Node* tmp[MAX_TOKENS]; int idx=0;
        for(int i=0;i<lpos;i++) tmp[idx++]=sent[i];
        for(int i=0;i<sent[lpos]->child_count;i++) tmp[idx++]=sent[lpos]->children[i];
        for(int i=lpos+1;i<count;i++) tmp[idx++]=sent[i];
        count=idx; 
        for(int i=0;i<count;i++) sent[i]=tmp[i];
    }
    printf("\n");
}
void rightmost_derivation(Node* root){
    ...
    while(1){
        ...
        printf("\n");
        int rpos=-1; 
        for(int i=count-1;i>=0;i--) if(sent[i]->child_count>0){rpos=i; break;}
        if(rpos==-1) break;
        Node* tmp[MAX_TOKENS]; int idx=0;
        for(int i=0;i<rpos;i++) tmp[idx++]=sent[i];
        for(int i=0;i<sent[rpos]->child_count;i++) tmp[idx++]=sent[rpos]->children[i];
        for(int i=rpos+1;i<count;i++) tmp[idx++]=sent[i];
        count=idx; 
        for(int i=0;i<count;i++) sent[i]=tmp[i];
    }
    printf("\n");
...
static int is_keyword(const char* s){
    return strcmp(s,"int")==0 || strcmp(s,"float")==0 || strcmp(s,"if")==0 ||
           strcmp(s,"else")==0 || strcmp(s,"while")==0 || strcmp(s,"print")==0;
}
    ...
    while((ch = fgetc(f)) != EOF){
        ...
        if(isalpha(ch) || ch == '_'){
            ...
            while((ch = fgetc(f)) != EOF && (isalnum(ch) || ch == '_')){
                if(len < MAX_VALUE - 1) lexeme[len++] = (char)ch;
            }
            lexeme[len] = '\0';
            if(ch != EOF) ungetc(ch, f);
            if(is_keyword(lexeme)) add_token("KEYWORD", lexeme, line);
            else add_token("IDENTIFIER", lexeme, line);
            continue;
        }
        ...
        if(ch == '&'){
            int next = fgetc(f);
            if(next == '&') add_token("LOGICAL", "&&", line);
            else {
                if(next != EOF) ungetc(next, f);
        ...
        if(ch == '='){
            int next = fgetc(f);
            if(next == '=') add_token("RELATIONAL", "==", line);
            else {
                if(next != EOF) ungetc(next, f);
...
void sym_scope_exit(void) {
    ...
    int i = sym_count - 1;
    while(i >= 0 && sym_table[i].scope == scope_depth) i--;
    sym_count = i + 1;
...
Symbol* sym_insert(const char* name, const char* type, int line) {
    if(sym_count >= MAX_SYM_TABLE) return NULL;
    Symbol* s = &sym_table[sym_count];
    strncpy(s->name, name, MAX_LABEL - 1); s->name[MAX_LABEL-1] = '\0';
    strncpy(s->type, type, MAX_LABEL - 1); s->type[MAX_LABEL-1] = '\0';
    s->scope  = scope_depth;
...
void print_symbol_table(void) {
    ...
    for(int i = 0; i < sym_count; i++) {
        printf("  %-16s  %-8s  %-7d  %-10d  %d\n",
               ...
               sym_table[i].scope, sym_table[i].offset,
               sym_table[i].line);
    }
    printf("================================================================================\n\n");
}
        ...
        for(int i = 0; i < sym_count; i++) {
            if(sym_table[i].scope == scope_depth) {
                printf("            %-12s  type=%-6s  offset=%d\n",
                       sym_table[i].name, sym_table[i].type, sym_table[i].offset);
                found = 1;
            }
        }
...
const char* infer_type(Node* node) {
    if(!node) { strcpy(infer_result, "unknown"); return infer_result; }

    if(strcmp(node->label, "INTEGER") == 0) { strcpy(infer_result, "int");   return infer_result; }
    if(strcmp(node->label, "FLOAT")   == 0) { strcpy(infer_result, "float"); return infer_result; }
    if(strcmp(node->label, "ID") == 0) {
        Symbol* s = sym_lookup(node->value);
        if(!s) {
            char msg[128];
            snprintf(msg, sizeof(msg), "use of undeclared variable '%s'", node->value);
    ...
    for(int i = 0; i < node->child_count; i++) {
        ...
        if(strcmp(ct, "float")   == 0) strcpy(infer_result, "float");
               ...
               strcmp(sym_table[i].name, var_name) == 0) {
                ...
                for(int j = 0; j < sym_count; j++)
                    if(sym_table[j].scope == scope_depth &&
                       strcmp(sym_table[j].name, var_name) == 0) cnt++;
                if(cnt > 1) { dup = 1; break; }
            }
        }
        if(dup) {
            char msg[128];
            snprintf(msg, sizeof(msg),
                     "multiple declaration of '%s' in scope %d", var_name, scope_depth);
            sem_error(msg, decl_line);
        } else {
            printf("  [DECL]   %-12s  type=%-6s  scope=%d  (line %d)\n",
                   var_name, decl_type, scope_depth, decl_line);
        }
...
/* Q3 — run_semantic_demo: demonstrates the two mandatory error cases.
 * Uses a temporary isolated sym_table state so it does not corrupt the
 * live symbol table the rest of the program uses. */
void run_semantic_demo(void) {
    printf("\n================================================================================\n");
    printf("  SEMANTIC ERROR DEMONSTRATION (2 Required Cases)\n");
    printf("================================================================================\n\n");
    printf("  The following cases are demonstrated against short inline programs\n");
    printf("  to satisfy the Q3 requirement of showing at least 2 error cases.\n\n");
    ...
    /* ---- Demo 1: Use of undeclared variable ---- */
    printf("  Demo program:  int a; a = 5; b = 10;\n");
    printf("  Expected error: use of undeclared variable 'b'\n\n");
    ...
    sym_insert("a", "int", 1);   /* int a — declared */
    /* a = 5 — lookup succeeds */
    /* b = 10 — undeclared */
    if(!sym_lookup("b"))
        sem_error("use of undeclared variable 'b'", 3);

    printf("  Result: %d semantic error(s) found.\n\n", sem_errors_count);

    /* ---- Demo 2: Multiple declarations in same scope ---- */
    printf("  Demo program:  int x; int x;\n");
    printf("  Expected error: multiple declaration of 'x' in scope 0\n\n");
    ...
    sym_insert("x", "int", 1);   /* first int x — OK */
    /* second int x — duplicate */
    if(sym_lookup_current("x")) {
        char msg[128];
        snprintf(msg, sizeof(msg),
                 "multiple declaration of 'x' in scope %d", scope_depth);
        sem_error(msg, 2);
    }
...
int main(void){
    if(!lexical_analyze_file("input.txt")) return 1;
    ...
    for(int i=0;i<stmt_count;i++){
        ...
        if(is_err || !stmt_ll1_ok[i] || !stmt_sr_ok[i]) total_errors_found++;
    }
    quiet_mode = 0;
    ...
    if(total_errors_found == 0){
        ...
        printf("  %d out of %d statement(s) have errors:\n\n", total_errors_found, stmt_count);
        for(int i=0;i<stmt_count;i++){
            int is_err = (strcmp(all_stmts[i]->label,"error_stmt")==0);
            if(is_err || !stmt_ll1_ok[i] || !stmt_sr_ok[i]){
                if(is_err && all_stmts[i]->rd_error[0] != '\0'){
                    printf("  %s\n", all_stmts[i]->rd_error);
                } else {
                    printf("  >> Statement %d (line %d): ", i+1, all_stmts[i]->line);
                    print_statement_source(all_stmts[i]);
                    printf("\n");
                }
                if(is_err) printf("     Reason: Syntax error (recursive descent parser failed)\n");
                else {
                    if(!stmt_ll1_ok[i]) printf("     Reason: LL(1) parser rejected\n");
                    if(!stmt_sr_ok[i])  printf("     Reason: Shift-Reduce parser rejected\n");
                }
                printf("\n");
            }
        }
...
*/
    ...
    /* ===== Q2: SYMBOL TABLE (phase5.c style) ===== */
    ...
    /* ===== Q3: SEMANTIC ANALYSIS (phase6.c style) ===== */
    ...
    printf("================================================================================\n");
    printf("  PHASE 6 - SEMANTIC ANALYSIS TRACE\n");
    printf("================================================================================\n\n");

    /* Walk the tree — prints [DECL]/[ASSIGN]/[IF]/[WHILE]/[PRINT]/[SEMANTIC ERROR] */
    ...
    printf("\n================================================================================\n");
    printf("SELECTED: "); print_statement_source(selected); printf("\n");
    printf("================================================================================\n\n");
    ...
    if(both_ok){
        printf("================================================================================\n");
        printf("BOTH PARSERS ACCEPTED - Showing derivations and statement parse tree\n");
        printf("================================================================================\n\n");
        printf("--- LEFTMOST DERIVATION ---\n");
        ...
        printf("\n");
        if(!ll1_ok || !sr_ok) {
            printf("  Parse tree and derivations are only shown for syntactically correct\n");
