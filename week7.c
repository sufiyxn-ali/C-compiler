...
typedef struct Node {
  ...
  int child_count;
  int line;
  int scope;  /* scope info for TAC generation */
  char source[4096];
  char rd_error[256];
} Node;
...
/* Forward declarations for Phase 7 - Three-Address Code */
typedef struct {
    char op[16];
    char arg1[MAX_VALUE];
    char arg2[MAX_VALUE];
...
void emit_label(const char* lbl);
char* get_mangled_name(const char* name, int scope);
char* gen_expr(Node* node);
void gen_bool(Node* node, const char* Ltrue, const char* Lfalse);
void gen_stmt(Node* node);
void gen_stmt_list(Node* node);
...
void print_tac_line(FILE* out, int i);
void print_quads(FILE* out);
void save_space_separated(const char* filename);
void collect_tac_statements(Node* root);
void node_source_str(Node* node, char* buf);
void explain_step_by_step(Node* node, int q_start, int q_end,
                           int t_start, int l_start, FILE* out);
int has_initialization(Node* node);
int generates_tac(Node* node);
...
/* ========================================================================== */
/* ===== PHASE 7: THREE-ADDRESS CODE (INTERMEDIATE CODE GENERATION) ========= */
/* ========================================================================== */
...
void emit(const char* op, const char* a1, const char* a2, const char* res){
    ...
    strcpy(quads[quad_count].op,     op  ? op  : "");
    strcpy(quads[quad_count].arg1,   a1  ? a1  : "_");
    strcpy(quads[quad_count].arg2,   a2  ? a2  : "_");
    strcpy(quads[quad_count].result, res ? res : "_");
    quad_count++;
}
...
/* Helper function to get mangled name with scope for TAC */
char* get_mangled_name(const char* name, int scope) {
    static char mangled[MAX_LABEL * 2];
    if (scope == 0) {
        sprintf(mangled, "%s", name); 
    } else {
        sprintf(mangled, "%s_s%d", name, scope);
    }
    return mangled;
}
...
/* 3AC for expressions */
char* gen_expr(Node* node){ 
    if (!node) { char* s=(char*)malloc(2); strcpy(s,"_"); return s; }

    if (strcmp(node->label,"INTEGER")==0 ||
        strcmp(node->label,"FLOAT")==0) {
        char* s=(char*)malloc(strlen(node->value)+1);
        strcpy(s,node->value);
        return s;
    }

    if (strcmp(node->label,"BOOLEAN")==0) {
        /* Convert true/false to 1/0 for TAC */
        char* s=(char*)malloc(2);
        strcpy(s, strcmp(node->value,"true")==0 ? "1" : "0");
        return s;
    }
    ...
    if (strcmp(node->label,"ID")==0) {
        Symbol* s = sym_lookup(node->value);
        int sc = (s) ? s->scope : 0;
        char* mangled = get_mangled_name(node->value, sc);
        char* result = (char*)malloc(strlen(mangled)+1);
        strcpy(result, mangled);
        return result;
    }

    if (strcmp(node->label,"factor")==0) {
        if (node->child_count >= 1) return gen_expr(node->children[0]);
        char* s=(char*)malloc(2); strcpy(s,"_"); return s;
    }

    /* bool_expr with single child (pass-through) */
    if (strcmp(node->label,"bool_expr")==0 && node->child_count==1) {
        return gen_expr(node->children[0]);
    }

    if ((strcmp(node->label,"term")==0 || strcmp(node->label,"expr")==0)
        && node->child_count == 3) {
        char* l = gen_expr(node->children[0]);
        char* r = gen_expr(node->children[2]);
...
static int is_logical(Node* n, const char* op){
    if (!n) return 0;
    for (int i=0;i<n->child_count;i++){
        Node* c = n->children[i];
        if ((strcmp(c->label,"LOGICAL")==0 || strcmp(c->label,"LOGICAL_AND")==0 ||
             strcmp(c->label,"LOGICAL_OR")==0) && strcmp(c->value,op)==0)
            return i;
    }
    return 0;
}
static int has_not_tac(Node* n){
    if (!n || n->child_count<1) return 0;
    return strcmp(n->children[0]->label,"NOT")==0;
}
static int find_relop_idx(Node* n){
    if (!n) return -1;
    for (int i=0;i<n->child_count;i++)
        if (strcmp(n->children[i]->label,"REL_OP")==0) return i;
    return -1;
}
...
void gen_bool(Node* node, const char* Ltrue, const char* Lfalse){
    ...
    if (strcmp(node->label,"bool_expr")==0 && node->child_count==1 &&
      strcmp(node->children[0]->label,"bool_expr")==0) {
        gen_bool(node->children[0], Ltrue, Lfalse);
        return;
    }

    if (has_not_tac(node)) {
        gen_bool(node->children[1], Lfalse, Ltrue);
        return;
    }

    int orIdx = is_logical(node,"||");
    if (orIdx > 0) {
        char* Lnext = new_label();
        gen_bool(node->children[0], Ltrue, Lnext);
        emit_label(Lnext);
        gen_bool(node->children[2], Ltrue, Lfalse);
        return;
    }

    int andIdx = is_logical(node,"&&");
    if (andIdx > 0) {
        char* Lnext = new_label();
        gen_bool(node->children[0], Lnext, Lfalse);
        emit_label(Lnext);
        gen_bool(node->children[2], Ltrue, Lfalse);
        return;
    }

    int rIdx = find_relop_idx(node);
    if (rIdx > 0 && node->child_count >= rIdx+2) {
        char* l = gen_expr(node->children[rIdx-1]);
        char* r = gen_expr(node->children[rIdx+1]);
        const char* op = node->children[rIdx]->value;
        char* t = new_temp();
        emit(op, l, r, t);
        emit("if", t, "_", Ltrue);
        emit("goto","_","_", Lfalse);
        return;
    }
...
void gen_stmt_list(Node* node){
    if (!node) return;
    for (int i = 0; i < node->child_count; i++) gen_stmt(node->children[i]);
}

void gen_block(Node* node){
    if (!node) return;

    /* Save current scope and enter new scope for TAC generation */
    int saved_scope = scope_depth;
    scope_depth++;

    for (int i = 0; i < node->child_count; i++){
        Node* c = node->children[i];
        if (strcmp(c->label,"stmt_list")==0) gen_stmt_list(c);
        else if (strcmp(c->label,"block")==0) gen_block(c);
        else gen_stmt(c);
    }
...
void gen_decl(Node* node){
    ...
    for (int i=0;i<node->child_count;i++){
        ...
        if (strcmp(c->label,"ID")==0) {
            ...
            init = node->children[i+1];
    }
    if (init){
        char* rhs = gen_expr(init);
        Symbol* s = sym_lookup(var);
        int sc = (s) ? s->scope : scope_depth;
        char* mangled = get_mangled_name(var, sc);
        emit("=", rhs, "_", mangled);
    }
}
...
void gen_if(Node* node){
    Node* cond=NULL; Node* thenB=NULL; Node* elseB=NULL;
    int else_seen=0;
    for (int i=0;i<node->child_count;i++){
        Node* c=node->children[i];
        if (strcmp(c->label,"bool_expr")==0) cond=c;
        else if (strcmp(c->label,"block")==0){
            if (!else_seen) thenB=c; else elseB=c;
        } else if (strcmp(c->label,"ELSE")==0) else_seen=1;
    }

    char* Lt = new_label();
    char* Lf= new_label();
    char* Lend  = elseB ? new_label() : Lf;
...
void gen_while(Node* node){
    Node* cond=NULL; Node* body=NULL;
    for (int i=0;i<node->child_count;i++){
        Node* c=node->children[i];
        if (strcmp(c->label,"bool_expr")==0) cond=c;
        else if (strcmp(c->label,"block")==0) body=c;
    }
    ...
    emit_label(Lbegin);
    gen_bool(cond, Lbody, Lend);
    emit_label(Lbody);
    gen_block(body);
    emit("goto","_","_", Lbegin);
    emit_label(Lend);
}
...
void gen_print(Node* node){
    ...
    for (int i=0;i<node->child_count;i++){
        Node* c=node->children[i];
        if (strcmp(c->label,"expr")==0 ||
            strcmp(c->label,"term")==0 ||
            strcmp(c->label,"factor")==0 ||
            strcmp(c->label,"bool_expr")==0 ||
            strcmp(c->label,"INTEGER")==0||
            strcmp(c->label,"FLOAT")==0  ||
            strcmp(c->label,"BOOLEAN")==0 ||
            strcmp(c->label,"ID")==0) { e=c; break; }
    }
    char* v = gen_expr(e);
    emit("param", v, "_", "_");
    emit("call",  "print", "1", "_");
...
void gen_stmt(Node* node){
    if (!node) return;
    if (strcmp(node->label,"decl_stmt")==0)   { gen_decl(node);   return; }
    if (strcmp(node->label,"assign_stmt")==0) { gen_assign(node); return; }
    if (strcmp(node->label,"if_stmt")==0)     { gen_if(node);     return; }
    if (strcmp(node->label,"while_stmt")==0)  { gen_while(node);  return; }
    if (strcmp(node->label,"print_stmt")==0)  { gen_print(node);  return; }
    if (strcmp(node->label,"block")==0)       { gen_block(node);  return; }
    if (strcmp(node->label,"stmt_list")==0)   { gen_stmt_list(node); return; }
    for (int i=0;i<node->child_count;i++) gen_stmt(node->children[i]);
}
...
void print_tac_line(FILE* out, int i){
  ...
  if (strcmp(q->op, "nop") == 0)
    return;
    if      (strcmp(q->op,"label")==0)    fprintf(out,"%s:\n", q->result);
    else if (strcmp(q->op,"goto")==0)     fprintf(out,"    goto %s\n", q->result);
    else if (strcmp(q->op,"if")==0)       fprintf(out,"    if %s goto %s\n", q->arg1, q->result);
    else if (strcmp(q->op,"ifFalse")==0)  fprintf(out,"    ifFalse %s goto %s\n", q->arg1, q->result);
    else if (strcmp(q->op,"=")==0)        fprintf(out,"    %s = %s\n", q->result, q->arg1);
    else if (strcmp(q->op,"param")==0)    fprintf(out,"    param %s\n", q->arg1);
    else if (strcmp(q->op,"call")==0)     fprintf(out,"    call %s, %s\n", q->arg1, q->arg2);
...
void print_quads(FILE* out){
    fprintf(out,"================================================================================\n");
    fprintf(out," index | op       | arg1            | arg2    | result \n");
    fprintf(out,"================================================================================\n");
    for (int i=0;i<quad_count;i++){
        fprintf(out," %4d  | %-8s | %-15s | %-7s | %-15s\n",
...
/* Selectable statements for step-by-step viewer */
#define MAX_SELECTABLE 200
Node* selectable[MAX_SELECTABLE];
int   selectable_count = 0;

int has_initialization(Node* node) {
    if (strcmp(node->label, "decl_stmt") != 0) return 0;
    for (int i = 0; i < node->child_count; i++) {
        if (strcmp(node->children[i]->label, "ASSIGNMENT") == 0)
            return 1;
    }
    return 0;
}
...
int generates_tac(Node* node) {
    if (!node) return 0;
    if (strcmp(node->label, "if_stmt") == 0) return 1;
    if (strcmp(node->label, "while_stmt") == 0) return 1;
    if (strcmp(node->label, "assign_stmt") == 0) return 1;
    if (strcmp(node->label, "print_stmt") == 0) return 1;
    if (strcmp(node->label, "decl_stmt") == 0) {
        return has_initialization(node);
    }
    return 0;
}
...
void collect_tac_statements(Node* root){
    ...
    if (generates_tac(root) && selectable_count < MAX_SELECTABLE) {
        selectable[selectable_count++] = root;
    }

    for (int i = 0; i < root->child_count; i++) {
        collect_tac_statements(root->children[i]);
    }
}

void node_source_str(Node* node, char* buf){
    buf[0]='\0';
    typedef struct { Node* n; } Frame;
    Frame stack[512]; int top=0;
    stack[top++].n = node;
    while (top>0){
        Node* cur = stack[--top].n;
        if (!cur) continue;
        if (cur->value[0]!='\0'){
            strcat(buf, cur->value);
            strcat(buf, " ");
        }
        for (int i=cur->child_count-1;i>=0;i--)
            stack[top++].n = cur->children[i];
    }
    int len=strlen(buf);
...
void explain_step_by_step(Node* node, int q_start, int q_end,
                           int t_start, int l_start, FILE* out){
    fprintf(out,"\n================================================================================\n");
    fprintf(out,"  STEP-BY-STEP TRANSLATION\n");
    fprintf(out,"================================================================================\n");

    char src[4096]={0};
    node_source_str(node, src);
    fprintf(out,"  Source statement: %s\n\n", src);
    fprintf(out,"  Statement type  : %s\n\n", node->label);

    int temps_found[100] = {0};
    int labels_found[100] = {0};
    int max_temp = 0;
    int max_label_found = 0;

    for (int i = q_start; i < q_end; i++) {
        Quad* q = &quads[i];

        if (q->arg1[0] == 't' && isdigit(q->arg1[1])) {
            int num = atoi(q->arg1 + 1);
            temps_found[num] = 1;
            if (num > max_temp) max_temp = num;
        }
        if (q->arg2[0] == 't' && isdigit(q->arg2[1])) {
            int num = atoi(q->arg2 + 1);
            temps_found[num] = 1;
            if (num > max_temp) max_temp = num;
        }
        if (q->result[0] == 't' && isdigit(q->result[1])) {
            int num = atoi(q->result + 1);
            temps_found[num] = 1;
            if (num > max_temp) max_temp = num;
        }

        if (q->arg1[0] == 'L' && isdigit(q->arg1[1])) {
            int num = atoi(q->arg1 + 1);
            labels_found[num] = 1;
            if (num > max_label_found) max_label_found = num;
        }
        if (q->arg2[0] == 'L' && isdigit(q->arg2[1])) {
            int num = atoi(q->arg2 + 1);
            labels_found[num] = 1;
            if (num > max_label_found) max_label_found = num;
        }
        if (q->result[0] == 'L' && isdigit(q->result[1])) {
            int num = atoi(q->result + 1);
            labels_found[num] = 1;
            if (num > max_label_found) max_label_found = num;
        }
    ...
    fprintf(out,"  Temporaries allocated : ");
    int first = 1;
    ...
    fprintf(out,"  Labels allocated      : ");
    first = 1;
    for (int i = 1; i <= max_label_found; i++) {
        if (labels_found[i]) {
            if (!first) fprintf(out,", ");
            fprintf(out,"L%d", i);
            first = 0;
    ...
    fprintf(out,"\n  Quadruples generated (indices %d to %d):\n\n", q_start, q_end-1);
    fprintf(out,"  %5s | %-8s | %-15s | %-7s | %-15s\n",
            "Index","op","arg1","arg2","result");
    fprintf(out,"  %5s-+-%s-+-%s-+-%s-+-%s\n",
            "-----","--------","---------------","-------","---------------");
    for (int i=q_start; i<q_end; i++){
        fprintf(out,"  %5d | %-8s | %-15s | %-7s | %-15s\n",
                i, quads[i].op, quads[i].arg1, quads[i].arg2, quads[i].result);
    }
...
/* ====== PHASE 8: OPTIMIZATIONS & TARGET CODE GENERATION ====== */

int constant_folds = 0;
int copy_props = 0;
int dead_code_elims = 0;
...
int is_constant_number(const char* s) {
  if (!s || strcmp(s, "_") == 0) return 0;
  char *endptr;
  strtol(s, &endptr, 10);
...
static int binding_find(OptBinding* bindings, int binding_count, const char* name) {
  for (int i = 0; i < binding_count; i++) {
    if (bindings[i].valid && strcmp(bindings[i].name, name) == 0)
      return i;
  }
  return -1;
}

static void binding_forget(OptBinding* bindings, int* binding_count, const char* name) {
  int idx = binding_find(bindings, *binding_count, name);
  if (idx >= 0) {
    bindings[idx].valid = 0;
    return;
  }
}

static void binding_set(OptBinding* bindings, int* binding_count, const char* name,
                        const char* value) {
  int idx = binding_find(bindings, *binding_count, name);
  if (idx < 0) {
    idx = *binding_count;
  ...
  strncpy(bindings[idx].name, name, MAX_LABEL - 1);
  bindings[idx].name[MAX_LABEL - 1] = '\0';
  strncpy(bindings[idx].value, value, MAX_VALUE - 1);
  bindings[idx].value[MAX_VALUE - 1] = '\0';
  bindings[idx].valid = 1;
}

static int resolve_operand(OptBinding* bindings, int binding_count,
                           const char* input, char* output, int depth) {
  if (!input) {
    strcpy(output, "_");
    return 0;
  }
  ...
  if (is_constant_number(input)) {
    strcpy(output, input);
    return 1;
  }
  if (depth > binding_count + 1) {
    strcpy(output, input);
    return 0;
  }
  int idx = binding_find(bindings, binding_count, input);
  if (idx < 0) {
    strcpy(output, input);
    return 0;
  }
  if (strcmp(bindings[idx].value, input) == 0) {
    strcpy(output, input);
    return 0;
  }
  return resolve_operand(bindings, binding_count, bindings[idx].value, output,
                         depth + 1);
}

static int eval_binary_op(const char* op, const char* lhs, const char* rhs,
                         char* out) {
  int left = atoi(lhs);
  int right = atoi(rhs);
  int result = 0;
...
static int optimize_quads_once(void) {
  OptBinding bindings[MAX_QUADS];
  int binding_count = 0;
  int changed = 0;
  ...
  for (int i = 0; i < quad_count; i++) {
    ...
    char resolved1[MAX_VALUE];
    char resolved2[MAX_VALUE];
    int was_const1 = resolve_operand(bindings, binding_count, q->arg1,
                                     resolved1, 0);
    int was_const2 = resolve_operand(bindings, binding_count, q->arg2,
                                     resolved2, 0);

    if (strcmp(q->arg1, resolved1) != 0) {
      strcpy(q->arg1, resolved1);
      changed = 1;
    }
    if (strcmp(q->arg2, resolved2) != 0) {
      strcpy(q->arg2, resolved2);
      changed = 1;
    }
    ...
    if (strcmp(q->op, "=") == 0) {
      if (strcmp(q->arg1, q->result) == 0) {
        quad_clear(q);
        changed = 1;
        binding_forget(bindings, &binding_count, q->result);
        continue;
      }
      if (is_constant_number(q->arg1)) {
        binding_set(bindings, &binding_count, q->result, q->arg1);
      } else if (!is_temp_name(q->arg1)) {
        binding_set(bindings, &binding_count, q->result, q->arg1);
      } else {
        binding_forget(bindings, &binding_count, q->result);
      }
    ...
    if (strcmp(q->op, "+") == 0 || strcmp(q->op, "-") == 0 ||
        ...
        strcmp(q->op, "<") == 0 || strcmp(q->op, "==") == 0 ||
        strcmp(q->op, "!=") == 0) {
      char folded[MAX_VALUE];
      if (was_const1 && was_const2 && eval_binary_op(q->op, q->arg1, q->arg2,
                                                     folded)) {
        if (strcmp(q->op, ">") == 0 || strcmp(q->op, "<") == 0 ||
            strcmp(q->op, "==") == 0 || strcmp(q->op, "!=") == 0) {
          printf("  Folding: %s = %s %s %s -> %s = %s\n",
                 q->result, q->arg1, q->op, q->arg2, q->result, folded);
        } else {
          printf("  Folding: %s = %s %s %s -> %s = %s\n",
                 q->result, q->arg1, q->op, q->arg2, q->result, folded);
        }
        strcpy(q->op, "=");
        strcpy(q->arg1, folded);
        q->arg2[0] = '\0';
        constant_folds++;
        binding_set(bindings, &binding_count, q->result, folded);
        changed = 1;
        continue;
      }
      binding_forget(bindings, &binding_count, q->result);
...
static int eliminate_dead_temp_quads(void) {
  int use_count[MAX_QUADS];
  ...
  for (int i = 0; i < quad_count; i++) {
    ...
    for (int j = 0; j < quad_count; j++) {
      if (strcmp(quads[i].result, quads[j].arg1) == 0)
        use_count[i]++;
      if (strcmp(quads[i].result, quads[j].arg2) == 0)
        use_count[i]++;
    }
  }

  Quad compacted[MAX_QUADS];
  int new_count = 0;
  int changed = 0;
  int removed = 0;
  ...
  for (int i = 0; i < quad_count; i++) {
    Quad* q = &quads[i];
    int pure_compute = (strcmp(q->op, "=") == 0 || strcmp(q->op, "+") == 0 ||
                        strcmp(q->op, "-") == 0 || strcmp(q->op, "*") == 0 ||
                        strcmp(q->op, "/") == 0 || strcmp(q->op, "%") == 0 ||
                        strcmp(q->op, ">") == 0 || strcmp(q->op, "<") == 0 ||
                        strcmp(q->op, "==") == 0 || strcmp(q->op, "!=") == 0);
    if (strcmp(q->op, "nop") == 0)
      continue;
    if (pure_compute && is_temp_name(q->result) && use_count[i] == 0) {
      changed = 1;
      removed++;
      continue;
    }
    compacted[new_count++] = *q;
  }
  ...
  if (changed) {
    ...
    dead_code_elims += removed;
  }
  return changed;
}

static void recompute_temp_count(void) {
  int max_temp = 0;
  for (int i = 0; i < quad_count; i++) {
    Quad* q = &quads[i];
    const char* fields[] = {q->arg1, q->arg2, q->result};
    for (int j = 0; j < 3; j++) {
      if (fields[j][0] == 't' && isdigit((unsigned char)fields[j][1])) {
        int temp_no = atoi(fields[j] + 1);
        if (temp_no > max_temp)
          max_temp = temp_no;
      }
    }
...
void run_phase8_optimizations(void) {
  int changed = 1;
  int iterations = 0;
  while (changed && iterations < 12) {
    changed = optimize_quads_once();
    changed |= eliminate_dead_temp_quads();
    iterations++;
  }
  recompute_temp_count();
}

void generate_target_code(FILE* out) {
  fprintf(out, ".DATA\n");
  for (int i = 0; i < sym_count; i++) {
    if (strcmp(sym_table[i].type, "int") == 0) {
  ...
  for (int i = 0; i < quad_count; i++) {
    ...
    if (strcmp(q->op, "label") == 0) {
      ...
      fprintf(out, "    MOV R0, %s\n", q->arg1);
      fprintf(out, "    MOD R0, %s\n", q->arg2);
      fprintf(out, "    MOV %s, R0\n", q->result);
    } else if (strcmp(q->op, ">") == 0 || strcmp(q->op, "<") == 0 || strcmp(q->op, "==") == 0 || strcmp(q->op, "!=") == 0) {
      fprintf(out, "    MOV R0, %s\n", q->arg1);
      fprintf(out, "    CMP R0, %s\n", q->arg2);
      if (strcmp(q->op, ">") == 0) fprintf(out, "    JG %s_true\n", q->result);
      else if (strcmp(q->op, "<") == 0) fprintf(out, "    JL %s_true\n", q->result);
      else if (strcmp(q->op, "==") == 0) fprintf(out, "    JE %s_true\n", q->result);
      else if (strcmp(q->op, "!=") == 0) fprintf(out, "    JNE %s_true\n", q->result);
      fprintf(out, "    MOV %s, #0\n", q->result);
      fprintf(out, "    JMP %s_end\n", q->result);
      fprintf(out, "%s_true:\n", q->result);
      fprintf(out, "    MOV %s, #1\n", q->result);
      fprintf(out, "%s_end:\n", q->result);
...
int main(int argc, char *argv[]) {
  const char *source_file = (argc > 1) ? argv[1] : "input.txt";

  if (!lexical_analyze_file(source_file))
    return 1;
  ...
  /* ========================================================================= */
  /* ===== PHASE 7: THREE-ADDRESS CODE GENERATION ===========================  */
  /* ========================================================================= */

  if (error_count > 0) {
    printf("\n====================================================================="
           "===========\n");
    printf("  PHASE 7 - TAC generation skipped due to syntax errors.\n");
    printf("====================================================================="
           "===========\n");
  } else {
    printf("\n====================================================================="
           "===========\n");
    printf("  PHASE 7 - INTERMEDIATE CODE GENERATION (Three-Address Code / Quadruples)\n");
    printf("====================================================================="
           "===========\n\n");

    printf("  REPRESENTATION FORMAT: QUADRUPLES\n");
    printf("  Each instruction is stored as a 4-tuple:\n");
    printf("      ( op , arg1 , arg2 , result )\n\n");


    selectable_count = 0;
    collect_tac_statements(tree);

    int stmt_quad_start[MAX_STMTS];
    int stmt_quad_end[MAX_STMTS];
    int stmt_temp_start[MAX_STMTS];
    int stmt_label_start[MAX_STMTS];
    ...
    for (int i = 0; i < selectable_count; i++){
        temp_count = 0;
        label_count = 0;
        stmt_quad_start[i] = quad_count;
        stmt_temp_start[i] = temp_count;
        stmt_label_start[i] = label_count;
        gen_stmt(selectable[i]);
        stmt_quad_end[i] = quad_count;
    }

    /* Save per-statement quads */
    Quad saved_quads[MAX_QUADS];
    int saved_quad_count = quad_count;
    for (int i = 0; i < quad_count; i++) {
        saved_quads[i] = quads[i];
    }
    ...
    printf("\n");
    print_quads(stdout);
    printf("\n  Summary: %d quadruples,  %d temporaries,  %d labels.\n",
           quad_count, temp_count, label_count);

    save_space_separated("Phase7_quadruples.txt");

    printf("\n");
    printf("================================================================================\n");
    printf("  THREE-ADDRESS CODE (Full Program)\n");
    printf("================================================================================\n\n");
    for (int i = 0; i < quad_count; i++) {
        print_tac_line(stdout, i);
    }
    printf("\n================================================================================\n");
    ...
    /* ===== PHASE 8 ===== */
    printf("\n================================================================================\n");
    printf("  PHASE 8 - OPTIMIZATION AND TARGET CODE GENERATION\n");
    printf("================================================================================\n\n");

    Quad phase8_quads[MAX_QUADS];
    int phase8_quad_count = quad_count;
    for (int i = 0; i < quad_count; i++) {
      phase8_quads[i] = quads[i];
    }

    int saved_constant_folds = constant_folds;
    int saved_copy_props = copy_props;
    int saved_dead_code_elims = dead_code_elims;
    constant_folds = 0;
    copy_props = 0;
    dead_code_elims = 0;

    printf("  [1] Iterative optimization (constant folding + copy propagation + DCE)\n");
    run_phase8_optimizations();
    ...
    if (asm_file) {
      generate_target_code(asm_file);
      fclose(asm_file);
      printf("\n  Target assembly code written to output.asm\n");
    }

    printf("\n================================================================================\n");
    printf("  TARGET CODE\n");
    printf("================================================================================\n\n");
    generate_target_code(stdout);

    printf("\n================================================================================\n");
    printf("  OPTIMIZATION SUMMARY\n");
    printf("================================================================================\n");
    printf("  Constant Folding:   %2d operations\n", constant_folds);
    printf("  Copy Propagation:   %2d replacements\n", copy_props);
    printf("  Dead Code Elim.:    %2d quads\n", dead_code_elims);
    printf("  Total Optimizations: %2d\n", constant_folds + copy_props + dead_code_elims);
    printf("================================================================================\n");

    quad_count = phase8_quad_count;
    for (int i = 0; i < phase8_quad_count; i++) {
      quads[i] = phase8_quads[i];
    }
    constant_folds = saved_constant_folds;
    copy_props = saved_copy_props;
    dead_code_elims = saved_dead_code_elims;

    quad_count = saved_quad_count;
    for (int i = 0; i < saved_quad_count; i++) {
        quads[i] = saved_quads[i];
    }
    ...
    if (selectable_count == 0){
        ...
        printf("\n================================================================================\n");
        printf("  STEP-BY-STEP TRANSLATION VIEWER\n");
        printf("================================================================================\n");
        printf("  Select a statement to see its detailed translation:\n\n");

        for (int i = 0; i < selectable_count; i++){
            char src[4096] = {0};
            node_source_str(selectable[i], src);
            if (strlen(src) > 60) {
                src[57] = '.';
                src[58] = '.';
                src[59] = '.';
            ...
            printf("  [%2d]  %-16s  line %-4d  %s\n",
                   i + 1, selectable[i]->label, selectable[i]->line, src);
        }

        int tac_choice = 0;
        while (tac_choice < 1 || tac_choice > selectable_count){
            printf("\n  Enter statement number (1-%d): ", selectable_count);
            if (scanf("%d", &tac_choice) != 1){
                int c;
                while ((c = getchar()) != '\n' && c != EOF);
                tac_choice = 0;
            }
            else if (tac_choice < 1 || tac_choice > selectable_count){
                printf("  Invalid choice. Please enter a number between 1 and %d.\n", selectable_count);
            }
        }

        int idx = tac_choice - 1;
        explain_step_by_step(selectable[idx],
                             stmt_quad_start[idx],
                             stmt_quad_end[idx],
                             stmt_temp_start[idx],
                             stmt_label_start[idx],
                             stdout);
    }
  }
